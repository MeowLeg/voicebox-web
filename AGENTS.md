...
Vue 3 + TypeScript + Tailwind frontend for Voicebox TTS API. Uses Vue CLI with src/ directory structure.
...
- Vue 3 + TypeScript with `src/` directory
- Tailwind CSS for styling
- API calls go through Vue CLI devServer proxy (not direct to backend)
- Audio files served via `/voicebox-web/audio/{id}` through proxy
- Frontend polls `/voicebox-web/generate/{id}/status` for completion
...
- Use `splitText()` from `src/api/index.ts` to chunk long text (500 char segments)
